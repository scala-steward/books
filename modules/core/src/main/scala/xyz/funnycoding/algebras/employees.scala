...
final class LiveEmployees[F[_]: Sync: Logger: MonadThrow: GenUUID](els: ElasticClient)(implicit
    U: Functor[F],
    E: Executor[F])
    extends Employees[F] {
...
object LiveEmployees {
  def make[F[_]: Sync: Logger: MonadThrow: GenUUID](
      els: ElasticClient)(implicit U: Functor[F], E: Executor[F]): F[Employees[F]] =
    Sync[F].delay {
